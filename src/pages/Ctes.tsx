...
const Ctes = () => {
  ...
  const importXmlFile = async (file: File | null) => {
    ...
    try {
      ...
      // Ao invés de tentar persistir no servidor (pode estar com erro), apenas abre o diálogo com o formulário preenchido
      setForm((p) => ({
        ...p,
        ...payload,
        fluxoOrigem: "nfe",
      ...
      setDialogOpen(true);
      toast({ title: "Dados importados", description: "Revise os campos e clique em Salvar para criar o rascunho." });
    } catch (err: any) {
        ...
        <DialogContent className="bg-card border-border w-full max-w-4xl mx-4 sm:mx-auto max-h-[90vh] overflow-y-auto p-4 sm:p-6">
          ...
          {form.fluxoOrigem === "nfe" && showNfeTipoStep && (
            <div className="py-4">
              ...
              {/* XML: permitir anexar arquivo diretamente neste passo (gera rascunho) */}
              {nfeOrigemTipo === "xml" && (
                <div className="mt-5">
                  <label className="text-xs font-medium text-muted-foreground mb-1 block">Anexar XML da NF-e</label>
                  <div className="flex gap-2 items-center">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        setImportingXml(true);
                        try {
                          const txt = await selectedXmlFile.text();
                          const parser = new DOMParser();
                          const doc = parser.parseFromString(txt, "application/xml");
                          const extract = (tag: string) => doc.querySelector(tag)?.textContent?.trim() || "";
                          const extractFromParent = (parent: string, tag: string) => doc.querySelector(`${parent} ${tag}`)?.textContent?.trim() || "";
