  ...
  return (
    <div className="glass-card p-4 sm:p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xs sm:text-sm font-semibold text-foreground">Alertas Recentes</h3>
        {alerts.length > 0 && (
      ...
      {alerts.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 text-center">
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">Nenhum alerta</p>
          <p className="text-[10px] sm:text-xs text-muted-foreground/60 mt-1">Os alertas são gerados automaticamente com base nos seus cadastros</p>
        </div>
      ) : (
        <div className="space-y-2.5">
          {alerts.slice(0, 5).map((alert, i) => (
            <div key={i} className={`flex items-start gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-lg border ${variantStyles[alert.variant]} animate-slide-in`} style={{ animationDelay: `${i * 80}ms` }}>
              <AlertTriangle className="w-4 h-4 mt-0.5 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-medium text-foreground">{alert.title}</p>
                <p className="text-[10px] sm:text-xs text-muted-foreground mt-0.5">{alert.desc}</p>
              </div>
            </div>
          ))}
        </div>
